...
class ProductController extends Controller
{
	...
	{	
		$product = Product::where('name', $request->product)->where('store_id', Auth::user()->store_id)->first();
		$number = $product->number;
		if (($request->number > 0) && ($request->number < $number)) {
			$product->number = $number - $request->number;
			$product->save();
			return redirect()->route('store.show', Auth::user()->store_id)->with('alert', 'Xuất hàng thành công');
		} else {
			return redirect()->back()->with('alert', 'số lượng không hợp lệ, số cần nhập phải lớn hơn 0 và nhỏ hơn số hàng đang có');
		}
	}
	...
	public function productAdd(AddProductRequest $request, $id)
	{
		...
		$product->number = $number + $request->number;
		$product->save();
		return redirect()->route('store.show', Auth::user()->store_id)->with('alert', 'Cập nhật thành công');
	}
}
